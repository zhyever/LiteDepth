...
model=dict(
    ...
    init_cfg=dict(
        type='Pretrained', 
        checkpoint='nfs/mobileAI2022_final/distill/robust_1e-1/epoch_600.pth'),
    upsample_type='bilinear',
    distill=True,
    ema=False,
    teacher_depther_cfg=teacher_model_cfg,
    student_depther_cfg=student_model_cfg,
    distill_loss=dict(type='MarginLoss', loss_weight=distill_weight, loss_type='robust', valid_mask=False),
    train_cfg=dict(),
    test_cfg=dict(mode='whole'),
    fix_last=True,
)
