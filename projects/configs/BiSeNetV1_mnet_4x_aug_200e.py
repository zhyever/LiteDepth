...
model = dict(
    ...
    backbone=dict(
        type='BiSeNetV1',
        in_channels=3,
        context_channels=(24, 40, 96),
        spatial_channels=(12, 12, 12, 24),
        out_channels=48,
        out_indices=(0, 1, 2), #default
        # out_indices=(0,),
        backbone_cfg=dict(
            type='mmcls.MobileNetV3',
            arch='small',
        ...
        init_cfg=dict(
            type='Pretrained', 
            checkpoint='nfs/checkpoints/mobilenet_v3_small-8427ecf0.pth')),
    decode_head=dict(
        type='DenseDepthHeadMobile',
        scale_up=True,
        min_depth=1e-3,
        max_depth=40,
        in_channels=[48, 24, 24],
        up_sample_channels=[24, 24, 24],
        channels=24, # last one
...
train_pipeline = [
    ...
    dict(type='RandomCropV2', crop_size=[(384, 512), (480, 640)]), # 120, 160 -> 60, 80 -> 30, 40 -> 15, 20, -> 8, 10 -> 4, 5
