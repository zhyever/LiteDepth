...
class VNLLoss(nn.Module):
    ...
    def transfer_xyz(self, depth):
        _, _, h, w = depth.shape
        fx = self.fx * w / self.input_size[1]
        fy = self.fy * h / self.input_size[0]
    ...
    def forward(self, pred_depth, gt_depth, select=True):
        ...
        # witch
        pred_depth, gt_depth = gt_depth, pred_depth

        

        self.init_image_coor(gt_depth.shape[2], gt_depth.shape[3])
        ...
        # print("----------------------------")
        gt_normal = gt_normal.clone()
        dt_normal = dt_normal.clone()
        gt_normal[torch.isnan(dt_normal)] = 0.
        dt_normal[torch.isnan(dt_normal)] = 0.
        gt_normal = gt_normal.clone()
        dt_normal = dt_normal.clone()
        gt_normal[torch.isinf(dt_normal)] = 0.
        dt_normal[torch.isinf(dt_normal)] = 0.
        # print(gt_points)
        # print(dt_normal)
        ...
        loss = torch.mean(loss) if loss.size(0) > 0 else dt_normal.new_tensor(0.0)
