...
class CustomEMAHook(Hook):
    ...
    def before_run(self, runner):
        ...
        assert hasattr(model, "student_depther")
        # only do it at initial stage
        if runner.iter == 0:
            print("Clone all parameters of student to the ema_model...")
    ...
    def momentum_update(self, model, momentum):
        for (src_name, src_parm), (tgt_name, tgt_parm) in zip(
            model.student_depther.named_parameters(), model.ema_model.named_parameters()
        ):
