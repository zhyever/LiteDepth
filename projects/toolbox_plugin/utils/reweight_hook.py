...
class DistillReweightHook(Hook):
    def __init__(
        self,
        max_weight,
        momentum=0.999,
    ...
    def before_run(self, runner):
        ...
        if is_module_wrapper(model):
            model = model.module
        assert hasattr(model, "distill_loss")
        # only do it at initial stage
        if runner.iter == 0:
            print("Setting the weight for distill loss to zero...")
            model.distill_loss.loss_weight = 0

    def before_train_iter(self, runner):
        """Update the distill loss weight every self.interval iterations."""
        ...
        runner.log_buffer.output["distill_loss_weight"] = self.max_weight * momentum
    ...
    def momentum_update(self, model, momentum):
        model.distill_loss.loss_weight = self.max_weight * momentum
